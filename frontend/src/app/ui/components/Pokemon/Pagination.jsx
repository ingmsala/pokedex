...
export default function Pagination ({ page, next, searchType, searchPokemon }) {
  ...
  return (
    <section className='flex gap-4'>
      {
        (page > 0)
          ? <button onClick={handleBefore} className='px-2 py-1 rounded flex justify-center items-center bg-violet-300 hover:bg-violet-400 dark:bg-slate-900 dark:hover:bg-slate-950'>{'< back'}</button>
